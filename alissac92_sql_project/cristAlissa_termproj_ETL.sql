...
NEW Function update_applicant_dim_full for SCD2 Maintenance
...
-- Trigger function to update PermitAppFact when ApplicantDim is updated
...
-- Trigger after update on ApplicantDim
...
-- Insert data into PermitAppFact
BEGIN;
...
JOIN
    orlando_development."DateDim" dd ON (dd.Year = hs.Year AND dd.Month = TO_NUMBER(TO_CHAR(TO_DATE(hs.Month, 'Month'), 'MM'), '99'))
WHERE
    dd.dayofmonth = 1;
...
JOIN
    orlando_development."DateDim" dd ON (dd.Year = ws.Year AND dd.Month = TO_NUMBER(TO_CHAR(TO_DATE(ws.Month, 'Month'), 'MM'), '99'))
WHERE
    dd.dayofmonth = 1;
